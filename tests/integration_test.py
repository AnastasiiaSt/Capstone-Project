...
@pytest.fixture
def input_labels():
    test_inputs = pd.read_csv(os.path.join(Path.cwd(), 'tests', 'fixtures', 'fixture.csv'))
...
def test_train(input_path, output_path, input_features, input_labels):
    ...
    with runner.isolated_filesystem():
        ...
        tree = runner.invoke(train, ['--dataset_path', input_path, '--save_model_path', output_path, '--model', 'Random Forest'])
        assert tree.exit_code == 0
